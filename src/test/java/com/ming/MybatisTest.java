...
public class MybatisTest {
    ...
    @Test
    public void testQueryUserByMultyIds(){
        SqlSession sqlSession = sqlSessionFactory.openSession();
        try{
            List<Integer> moreIds = new ArrayList<Integer>();
            moreIds.add(10);
            moreIds.add(16);
            moreIds.add(24);
            UserBz userBz = new UserBz();
            userBz.setIds(moreIds);
            UserMapper userMapper = sqlSession.getMapper(UserMapper.class);
            List<User> userList = userMapper.queryUserByMultyIds(userBz);
    ...
    @Test
    public void testQueryUserByMultyIds2(){
        SqlSession sqlSession = sqlSessionFactory.openSession();
        try{
            List<Integer> moreIds = new ArrayList<Integer>();
            moreIds.add(10);
            ...
            userBz.setIds(moreIds);
            UserMapper userMapper = sqlSession.getMapper(UserMapper.class);
            List<User> userList = userMapper.queryUserByMultyIds2(userBz);
            System.out.println(userList);
        }catch (Exception ex){
            ex.printStackTrace();
        }
